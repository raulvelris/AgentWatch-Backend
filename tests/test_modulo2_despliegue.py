...
def test_rollback_genera_version_nueva_sin_borrar():
    ...
    client.post(f"/api/v1/agents/{ag}/deploy")
    client.post(f"/api/v1/agents/{ag}/deploy")
    antes = client.get(f"/api/v1/agents/{ag}/versions").json()["versions"]
    primera = antes[0]["id"]
    r = client.post(f"/api/v1/agents/{ag}/rollback/{primera}")
    assert r.status_code == 200
...
def test_rollback_version_inexistente_da_404():
    client.post(f"/api/v1/agents/{AGENTE}/deploy")
    r = client.post(f"/api/v1/agents/{AGENTE}/rollback/no-existe")
    assert r.status_code == 404


def test_promote_a_prod_sin_admin_es_403():
    # RF06: promotion a prod exige rol ADMIN.
    r = client.post(
...
def test_promote_no_prod_sin_admin_queda_pendiente():
    # RF06: promover a un ambiente != prod no exige ADMIN; queda 'pendiente'.
    r = client.post(
        f"/api/v1/agents/{AGENTE}/promote",
        json={
            "ambiente_origen": "dev",
...
def test_rollback_encadenado_sigue_append_y_una_vigente():
    ...
    client.post(f"/api/v1/agents/{ag}/deploy")  # v1
    client.post(f"/api/v1/agents/{ag}/deploy")  # v2
    v1 = client.get(f"/api/v1/agents/{ag}/versions").json()["versions"][0]["id"]
    r1 = client.post(f"/api/v1/agents/{ag}/rollback/{v1}")  # v3 (rollback)
    assert r1.status_code == 200
    v3 = r1.json()["version"]["id"]
    r2 = client.post(f"/api/v1/agents/{ag}/rollback/{v3}")  # v4 (rollback de un rollback)
